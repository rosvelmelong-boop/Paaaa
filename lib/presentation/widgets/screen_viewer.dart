...
class _ScreenViewerState extends State<ScreenViewer> {
  ...
  void initState() {
    ...
    _controller = WebViewController()
      ...
      ..setBackgroundColor(const Color(0xFF0A1628)) // Match DESIGN.md bg-canvas
  ...
  Future<void> _loadHtml() async {
    try {
      // Load the screen HTML and navigation interceptor JS from assets
        ...
        if (_isLoading)
          const Center(
            child: CircularProgressIndicator(
              valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF1D9A8A)), // PropVeil Teal
            ),
          ),
      ],
    );
